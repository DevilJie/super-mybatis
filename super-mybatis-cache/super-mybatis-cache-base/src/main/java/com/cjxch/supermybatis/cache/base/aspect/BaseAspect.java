...
public class BaseAspect {
    ...
    protected CacheEnable cacheEnable;
    ...
    public String processKeySet() throws Exception {
        ...
        if(expires == null || expires == 0l){
            PrintLog.DEBUG(logger, "The current call method is " + method.getName() + ", No expires time settings were found, Ready to search from @CacheEnable");
            expires = cacheEnable.expires();
            if(expires == null || expires == 0l){
    ...
    public String processKeyEvict() throws Exception {
        ...
        return processKey(evict.key());
    }


    private String processKey(String k){
    ...
    private Method loadInvokeMethod() throws Exception{
        ...
        cacheEnable = (CacheEnable) target.getAnnotation(CacheEnable.class);
        if(cacheEnable == null) throw new SuperMybatisException("continue");
